...
export default function Mtp({parent_container}) {
  ...
  const getMTP = async () => {
    ...
    const mtpPolicies = policiesArray.filter(policy => policy.category === 'comprehensive').sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        ...
        {shownPolicies.length > 0
         ...
         :
          <div className="no-table-data">
              <i><ImFilesEmpty /></i>
              <h4>No match</h4>
              <p>There is no current match for Comprehensive sticker</p>
          </div>
        }
